...
inline void print_error_and_terminate(std::stringstream &error_stream)
{
    ...
    throw diskann::ANNException(error_stream.str(), -1);
}
...
inline void load_truthset(const std::string &bin_file, uint32_t *&ids, float *&dists, size_t &npts, size_t &dim)
{
    ...
    if (diskann::debug) diskann::cout << "Reading truthset file " << bin_file.c_str() << " ..." << std::endl;
                                ...
                                uint64_t &gt_num)
{
    size_t read_blk_size = 64 * 1024 * 1024;
    cached_ifstream reader(bin_file, read_blk_size);
    if (diskann::debug) diskann::cout << "Reading truthset file " << bin_file.c_str() << "... " << std::flush;
    size_t actual_file_size = reader.get_file_size();
...
DISKANN_DLLEXPORT double calculate_mrr(unsigned num_queries, unsigned *gold_std, float *gs_dist, unsigned dim_gs,
                                          unsigned *our_results, unsigned mrr_at);
...
inline size_t save_bin(const std::string &filename, T *data, size_t npts, size_t ndims, size_t offset = 0)
{
    ...
    if (diskann::debug) diskann::cout << "Writing bin: " << filename.c_str() << std::endl;
    writer.seekp(offset, writer.beg);
    int npts_i32 = (int)npts, ndims_i32 = (int)ndims;
    size_t bytes_written = npts * ndims * sizeof(T) + 2 * sizeof(uint32_t);
    writer.write((char *)&npts_i32, sizeof(int));
    writer.write((char *)&ndims_i32, sizeof(int));
    if (diskann::debug) diskann::cout << "bin: #pts = " << npts << ", #dims = " << ndims << ", size = " << bytes_written << "B"
                  << std::endl;

    writer.write((char *)data, npts * ndims * sizeof(T));
    writer.close();
    if (diskann::debug) diskann::cout << "Finished writing bin." << std::endl;
                                  ...
                                  size_t &dim, size_t &rounded_dim)
{
    ...
    // diskann::cout << "Metadata: #pts = " << npts << ", #dims = " << dim << ", aligned_dim = " << rounded_dim << "... " << std::flush;
    size_t allocSize = npts * rounded_dim * sizeof(T);
    if (diskann::debug) diskann::cout << "allocating aligned memory of " << allocSize << " bytes... " << std::flush;
    alloc_aligned(((void **)&data), allocSize, 8 * sizeof(T));
    if (diskann::debug) diskann::cout << "done. Copying data to mem_aligned buffer..." << std::flush;
...
inline void load_aligned_bin(const std::string &bin_file, T *&data, size_t &npts, size_t &dim, size_t &rounded_dim)
{
    ...
    try
    {
        if (diskann::debug) diskann::cout << "Reading (with alignment) bin file " << bin_file << " ..." << std::flush;
        reader.open(bin_file, std::ios::binary | std::ios::ate);
