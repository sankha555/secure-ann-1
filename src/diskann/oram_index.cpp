...
OramIndex<T, LabelT>::OramIndex(std::shared_ptr<AlignedFileReader> &fileReader, diskann::Metric m)
...
template <typename T, typename LabelT> OramIndex<T, LabelT>::~OramIndex()
{
#ifndef EXEC_ENV_OLS
    if (data != nullptr)
    {
    ...
    if (_load_flag)
    {
        if (diskann::debug) diskann::cout << "Clearing scratch" << std::endl;
...
template <typename T, typename LabelT> inline uint64_t OramIndex<T, LabelT>::get_node_sector(uint64_t node_id)
{
...
template <typename T, typename LabelT>
inline char *OramIndex<T, LabelT>::offset_to_node(char *sector_buf, uint64_t node_id)
{
    return sector_buf + (_nnodes_per_sector == 0 ? 0 : (node_id % _nnodes_per_sector) * _max_node_len);
}

template <typename T, typename LabelT> inline uint32_t *OramIndex<T, LabelT>::offset_to_node_nhood(char *node_buf)
{
    return (unsigned *)(node_buf + _disk_bytes_per_point);
}

template <typename T, typename LabelT> inline T *OramIndex<T, LabelT>::offset_to_node_coords(char *node_buf)
{
    return (T *)(node_buf);
}

template <typename T, typename LabelT>
void OramIndex<T, LabelT>::setup_thread_data(uint64_t nthreads, uint64_t visited_reserve)
{
    if (diskann::debug) diskann::cout << "Setting up thread-specific contexts for nthreads: " << nthreads << std::endl;
...
template <typename T, typename LabelT>
std::vector<bool> OramIndex<T, LabelT>::read_nodes(const std::vector<uint32_t> &node_ids,
...
template <typename T, typename LabelT> void OramIndex<T, LabelT>::load_cache_list(std::vector<uint32_t> &node_list)
{
    if (diskann::debug) diskann::cout << "Loading the cache list into memory.." << std::flush;
    ...
    if (diskann::debug) diskann::cout << "..done." << std::endl;
}

#ifdef EXEC_ENV_OLS
template <typename T, typename LabelT>
void OramIndex<T, LabelT>::generate_cache_list_from_sample_queries(MemoryMappedFiles &files, std::string sample_bin,
...
#else
template <typename T, typename LabelT>
void OramIndex<T, LabelT>::generate_cache_list_from_sample_queries(std::string sample_bin, uint64_t l_search,
...
template <typename T, typename LabelT>
void OramIndex<T, LabelT>::cache_bfs_levels(uint64_t num_nodes_to_cache, std::vector<uint32_t> &node_list,
                                               const bool shuffle)
{
    ...
    if (diskann::debug) diskann::cout << "Caching " << num_nodes_to_cache << "..." << std::endl;
    ...
    while ((node_set.size() + cur_level->size() < num_nodes_to_cache) && cur_level->size() != 0)
    {
        ...
        if (diskann::debug) diskann::cout << "Level: " << lvl << std::flush;
...
template <typename T, typename LabelT> void OramIndex<T, LabelT>::use_medoids_data_as_centroids()
{
    if (_centroid_data != nullptr)
        aligned_free(_centroid_data);
   ...
   if (diskann::debug) diskann::cout << "Loading centroid data from medoids vector data of " << _num_medoids << " medoid(s)" << std::endl;
...
template <typename T, typename LabelT>
void OramIndex<T, LabelT>::generate_random_labels(std::vector<LabelT> &labels, const uint32_t num_labels,
...
template <typename T, typename LabelT>
std::unordered_map<std::string, LabelT> OramIndex<T, LabelT>::load_label_map(std::basic_istream<char> &map_reader)
{
...
template <typename T, typename LabelT>
LabelT OramIndex<T, LabelT>::get_converted_label(const std::string &filter_label)
{
...
template <typename T, typename LabelT>
void OramIndex<T, LabelT>::reset_stream_for_reading(std::basic_istream<char> &infile)
{
    infile.clear();
    infile.seekg(0);
}
...
template <typename T, typename LabelT>
void OramIndex<T, LabelT>::get_label_file_metadata(const std::string &fileContent, uint32_t &num_pts,
...
template <typename T, typename LabelT>
inline bool OramIndex<T, LabelT>::point_has_label(uint32_t point_id, LabelT label_id)
{
...
template <typename T, typename LabelT>
void OramIndex<T, LabelT>::parse_label_file(std::basic_istream<char> &infile, size_t &num_points_labels)
{
...
template <typename T, typename LabelT> void OramIndex<T, LabelT>::set_universal_label(const LabelT &label)
{
...
#ifdef EXEC_ENV_OLS
template <typename T, typename LabelT>
int OramIndex<T, LabelT>::load(MemoryMappedFiles &files, uint32_t num_threads, const char *index_prefix)
{
#else
template <typename T, typename LabelT> int OramIndex<T, LabelT>::load(uint32_t num_threads, const char *index_prefix)
{
#endif
...
#ifdef EXEC_ENV_OLS
template <typename T, typename LabelT>
int OramIndex<T, LabelT>::load_from_separate_paths(diskann::MemoryMappedFiles &files, uint32_t num_threads,
                                                      const char *index_filepath, const char *pivots_filepath,
                                                      const char *compressed_filepath)
{
#else
template <typename T, typename LabelT>
int OramIndex<T, LabelT>::load_from_separate_paths(uint32_t num_threads, const char *index_filepath,
                                                      const char *pivots_filepath, const char *compressed_filepath)
{
    ...
    if (diskann::debug) diskann::cout << "Loaded PQ centroids and in-memory compressed vectors. #points: " << _num_points
...
void OramIndex<T, LabelT>::oram_read(
    ...
    for(int i = 0; i < frontier_nhoods.size(); i++){
        ...
        DiskANNNode<T, LabelT>* node = node_map[node_id];
...
template <typename T, typename LabelT>
void OramIndex<T, LabelT>::cached_beam_search_with_oram(const T *query1, const uint64_t k_search, const uint64_t l_search,
                                                 uint64_t *indices, float *distances, const uint64_t beam_width,
                                                 const bool use_filter, const LabelT &filter_label,
                                                 const uint32_t io_limit, const bool use_reorder_data,
...
template <typename T, typename LabelT>
uint32_t OramIndex<T, LabelT>::range_search(const T *query1, const double range, const uint64_t min_l_search,
...
template <typename T, typename LabelT> uint64_t OramIndex<T, LabelT>::get_data_dim()
{
    return _data_dim;
}

template <typename T, typename LabelT> diskann::Metric OramIndex<T, LabelT>::get_metric()
{
    return this->metric;
}

#ifdef EXEC_ENV_OLS
template <typename T, typename LabelT> char *OramIndex<T, LabelT>::getHeaderBytes()
{
    IOContext &ctx = reader->get_ctx();
    AlignedRead readReq;
    readReq.buf = new char[OramIndex<T, LabelT>::HEADER_SIZE];
    readReq.len = OramIndex<T, LabelT>::HEADER_SIZE;
    readReq.offset = 0;
...
template <typename T, typename LabelT>
std::vector<std::uint8_t> OramIndex<T, LabelT>::get_pq_vector(std::uint64_t vid)
{
    std::uint8_t *pqVec = &this->data[vid * this->_n_chunks];
    return std::vector<std::uint8_t>(pqVec, pqVec + this->_n_chunks);
}

template <typename T, typename LabelT> std::uint64_t OramIndex<T, LabelT>::get_num_points()
{
    return _num_points;
}

// instantiations
template class OramIndex<uint8_t>;
template class OramIndex<int8_t>;
template class OramIndex<float>;
template class OramIndex<uint8_t, uint16_t>;
template class OramIndex<int8_t, uint16_t>;
template class OramIndex<float, uint16_t>;
template class OramIndex<uint8_t, node_id_t>;
template class OramIndex<int8_t, node_id_t>;
template class OramIndex<float, node_id_t>;
