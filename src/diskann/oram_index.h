...
template <typename T, typename LabelT = uint32_t> class OramIndex
{
  public:
    DISKANN_DLLEXPORT OramIndex(std::shared_ptr<AlignedFileReader> &fileReader,
                                   diskann::Metric metric = diskann::Metric::L2);
    DISKANN_DLLEXPORT ~OramIndex();
    ...
    DISKANN_DLLEXPORT void cached_beam_search_with_oram(const T *query, const uint64_t k_search, const uint64_t l_search,
                                            ...
                                            const bool use_filter, const LabelT &filter_label,
                                            const uint32_t io_limit, const bool use_reorder_data = false,
                                            QueryStats *stats = nullptr, OramAPI* oram = nullptr);
