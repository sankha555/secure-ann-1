...
double calculate_mrr(uint32_t num_queries, uint32_t *gold_std, float *gs_dist, uint32_t dim_gs,
    uint32_t *our_results, uint32_t mrr_at) 
{
    double mrr = 0;

    for (size_t i = 0; i < num_queries; i++) 
    {
        uint32_t *gt_vec = gold_std + dim_gs * i;  // Ground truth for query i
        uint32_t *res_vec = our_results + mrr_at * i;  // Our results for query i

        for (size_t rank = 0; rank < mrr_at; rank++) 
        {
            if (res_vec[rank] == *gt_vec) 
            {
                mrr += 1.0 / (rank + 1);
                goto next_query;
            }
